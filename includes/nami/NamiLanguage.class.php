...
class NamiLanguage {
	...
	public function __construct( $locale ) {
		...
		if( preg_match( '/^(\w+)_(\w+)\.([^.]+)$/', $locale, $m ) ) {
			...
			throw new NamiException( "Не удается распознать локаль '{$locale}'. Используйте корректную локаль, например, ru_RU.UTF-8." );
		}
	}
	...
	public function __get( $name ) {
		...
		throw new NamiException( "Обращение к несуществующему полю '{$name}' класса ".__CLASS__ );
	}
