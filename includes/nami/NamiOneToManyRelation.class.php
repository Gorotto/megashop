...
/**
    ...
    Эти сведения и хранятся экземпляром класса NamiOneToManyRelation.
    Пример связи: new NamiOneToManyRelation( 'Owner', 'owner_id', 'Car', 'cars' );
    В модели Owner будет поле cars для достаупа к RelatedNamiQuerySetу с амтомобилями
    А у модели Car будет поле owner_id с идентификатором владельца
*/

class NamiOneToManyRelation
{
    ...
    */
    ...
    {
        // Проверим, чтобы классы являлись моделями Nami
        foreach( array( $key_model, $set_model ) as $classname )
        {
            if( ! ( class_exists( $classname ) && is_subclass_of( $classname, 'NamiModel' ) ) )
            {
                throw new NamiException( "'{$classname}' is not a Nami model" );
            }
        }
