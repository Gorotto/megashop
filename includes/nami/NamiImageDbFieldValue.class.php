...
class NamiImageDbFieldValue extends NamiFileDbFieldValue {
    ...
    function __construct(array $params = array()) {
        ...
        if (is_array($params['variants'])) {
            ...
            if (array_key_exists('original', $params['variants'])) {
                throw new NamiException("Использование варианта изображения 'original' запрещено, это название зарезервировано для исходного изображения.");
            }
    ...
    protected function copyValue($source) {
        if (is_object($source)) {
            ...
            foreach (array_keys($this->variants) as $variant) {
                if (property_exists($source, $variant)) {
                    $this->value->$variant = new NamiImageVariant($source->$variant);
                }
            }
        }
    }
        ...
        if ($this->value) {
            ...
            foreach (array_keys($this->variants) as $variant) {
                if (!property_exists($this->value, $variant)) {
                    $this->value->$variant = new NamiImageVariant();
                }
                $vobject = $this->value->$variant;
                if (!$vobject->uri) {
                    $vobject->uri = "{$this->path}/{$variant}/{$this->name}";
                }
                $vobject->selfCheck();
            }
            $this->value->original = new NamiImageVariant();
    ...
    public function loadSource($source) {
        ...
        foreach (array_merge(array(''), array_keys($this->variants)) as $name) {
            ...
            if (!@file_exists($dir)) {
                if (!@mkdir($dir, 0755, true)) {
                    throw new NamiException("Невозможно создать каталог '{$dir}' для сохранения изображения.");
                }
            } else if (!is_writeable($dir)) {
                throw new NamiException("Невозможно сохранить изображение: запись в каталог '{$dir}' запрещена.");
            }
        }
        ...
        try {
            ...
            $value->original = new NamiImageVariant();
            $value->original->uri = "{$this->path}/{$this->name}";
            $value->original->selfCheck();

            if ($value->original->width > 2500 || $value->original->height > 2500) {
                throw new NamiException("Масштаб изображения превышает допустимые 2500px*2500px '{$this->source['name']}'.");
            }

            if (!( $value->original->width && $value->original->height )) {
                throw new NamiException("Файл '{$this->source['name']}' не является изображением.");
            }

            $path = "{$_SERVER['DOCUMENT_ROOT']}{$this->path}";
            foreach ($this->variants as $name => $options) {
                $v = new NamiImageVariant();
                list( $v->width, $v->height ) = $this->transformImage("{$path}/{$this->name}", "{$path}/{$name}/{$this->name}", $options);
                $v->size = filesize("{$path}/{$name}/{$this->name}");
                $value->$name = $v;
            }
            ...
            foreach (get_object_vars($value) as $name => $v) {
                @unlink($path . ( $name == 'original' ? '' : "/{$name}" ) . "/{$this->name}");
            }
            throw $e;
        }
    }
    ...
    function beforeSync() {
        ...
        foreach (array_keys($this->variants) as $name) {
            ...
            if (!@file_exists($dir)) {
                if (!@mkdir($dir, 0755, true)) {
                    throw new NamiException("Не удалось создать каталог хранения изображений '{$dir}'. Недостаточно привилегий доступа.");
                }
            } else if (!is_writeable($dir) && !( @chmod($dir, 0755) && is_writeable($dir) )) {
                throw new NamiException("Запись в каталог '{$dir}' запрещена.");
            }
        }
    }
    ...
    function checkImageFormat($type) {
        ...
        if (!array_key_exists($type, $typeCheck)) {
            throw new NamiException("Изображения в формате " . image_type_to_extension($type, false) . " не поддерживаются.");
        }

        if (!imagetypes() & $typeCheck[$type]) {
            throw new NamiException("Изображения в формате " . image_type_to_extension($type, false) . " не поддерживаются графической библиотекой сайта.");
        }
    ...
    function loadTransformOptions($defaults, $overrides) {
        ...
        switch (strtolower($opts['format'])) {
            ...
            case 'png':
                ...
                break;
            default:
                throw new NamiException("Формат {$opts['format']} неизвестен. Используйте jpeg, png или gif.");
                break;
        }
    ...
    function transformImage($src, $dst, array $args) {
        ...
        $opts = array(
            ...
            'enlarge' => false, // Увеличивать исходное изображение, если оно меньше указанных размеров
        ...
        try {
            ...
            throw new NamiException("Ошибка изменения размеров изображения {$src}. " . $e->getMessage());
        }

        // Проверим файл
        if (!file_exists($src)) {
            throw new NamiException("Файл {$src} не существует.");
        }
        if (!is_readable($src)) {
            throw new NamiException("Файл {$src} не доступен для чтения.");
        }
        ...
        if (!$oriW || !$oriH) {
            throw new NamiException("При чтении параметров изображения {$src} произошла ошибка. Файл не является изображением или поврежден.");
        }
        ...
        // Проверим режим увеличения и размеры картинок
        if (!$enlarge && !$spacefill) {
            if ($width && $height) {
                if ($proportional) {
                    if ($width / $height > $oriW / $oriH) {
                        // Ограничивающий параметр - высота исходной картинки
                        if ($height > $oriH) {
                            $height = $oriH;
                        }
                    } else {
                        // Ограничивающий параметр - ширина исходной картинки
                        if ($width > $oriW) {
                            $width = $oriW;
                        }
                    }
        ...
        if ($resW && $resH) {
            ...
            if ($crop || !$proportional) {
                ...
                //TODO: нужно проверять случаи когда размеры фотки больше
                //требуемых. и соотвественно обрабатывать варианты масштабирования
                //обрезания и т.п.
                $dstW = $resW;
                $dstH = $resH;
            } else {
        ...
        if ($watermark) {
            ...
            if (!file_exists($watermark)) {
                throw new NamiException("Файл водяного знака  {$watermark} не существует.");
            }
            if (!is_readable($watermark)) {
                throw new NamiException("Файл водяного знака {$watermark} недоступен для чтения.");
            }
            ...
            if (!$watW || !$watH) {
                throw new NamiException("При чтении параметров водяного знака {$watermark} произошла ошибка. Файл не является изображением или поврежден.");
            }
        ...
        switch ($format) {
            ...
            case IMAGETYPE_GIF:
                ...
                break;
            case IMAGETYPE_PNG:
                imagepng($img, $dst, $pngcompression, PNG_NO_FILTER);
                break;
        }
