 ...
 */
abstract class NamiDbField {

    protected $value = null; // Поле хранения данных
    protected $title = null; //название поля
    ...
    protected $valueClassname = 'NamiDbFieldValue'; // Имя класса значений
    ...
    function __construct(array $params = array()) {
        ...
        foreach ($params as $k => $v) {
            if ($k == 'value')
                throw new NamiException("Can't set field value through constructor");
            if (property_exists($this, $k))
                $this->$k = $v;
        }

        $this->value = new NamiDbFieldValueStorage($this->localized);
    ...
    public function __get($name) {
        if (!property_exists($this, $name))
            throw new NamiException("Unknown property '{$name}'");

        return $this->$name;
    }

    public function __set($name, $value) {
        if ($name == 'name' && is_null($this->name)) {
            $this->name = $value;
        }

        if ($name == 'name' && is_null($this->title)) {
            $this->title = $value;
        }
    ...
    function setValue($value, $language = null) {
        ...
        if ($this->null == false && is_null($value)) {
            throw new NamiValidationException("Cannot be null", $this);
        }
    ...
    function getValue($language = null) {
        $value = is_object($this->value) ? $this->value->get($language) : null;
        return is_null($value) ? null : $value->get();
    }
    ...
    /**
     *   Работа с флагом грязности поля
     */
