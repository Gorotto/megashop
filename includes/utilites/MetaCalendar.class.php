...
class MetaCalendar {
	...
	function __construct( $time = null ) {
		...
		$lastDayStart = mktime( 0, 0, 0, $month, $dayCount, $year );
		...
		for( $date = $start, $row = 0; $row < $rowCount; $row++ ) {
			for( $wday = 0; $wday < 7; $wday++, $date = strtotime( "+1 day", $date ) ) {
				$key = strftime( "%F", $date );
				$this->dayMap[ $key ] = (object) array( 
				    'date'      => $date,
				    'first'     => $date == $firstDay,
				    'last'      => $date == $lastDayStart,
				    'day'       => strftime( "%e", $date ),
				    'wday'      => $wday + 1,
				    'off'       => $wday >= 5,
				    'objects'   => array(),
				    'extra'     => ( $date < $firstDay || $date > $lastDay ),
				);
	...
	*/
	function attachObjects( array $objects, $dateFieldName ) {
		foreach( $objects as $obj ) {
            if (is_array($obj)) {
                if (array_key_exists($dateFieldName, $obj)) {
    				$this->dayMap[ $obj[$dateFieldName] ]->objects[] = $obj;
                }
