...
/**
*   Обертка к приятной функции load.
*   Link: http://www.bin-co.com/php/scripts/load/
*   Version : 3.00.A
*   Основная фича — позволяет одним HTTP-запросом получить заголовки и текст ответа.
*/
class UriLoadLibrary {
    protected $options;
    ...
    function load( $url, $options = null ) {
        if( is_null( $options ) ) {
        ...
        if($info['http_code'] == 404) {
            ...
            //Seperate header and content
            $header_text = substr($response, 0, array_key_exists( 'header_size', $info ) ? $info['header_size'] : 0);
            $body = substr($response, array_key_exists( 'header_size', $info ) ? $info['header_size'] : 0);
