CKEDITOR.plugins.add( 'typofilter',
    {
        init : function( editor )
        {
            ...
            editor.addCommand('typofilter', {
                exec:typofilterProcess
            });
        }
    } );
...
var typofilterProcess = function (e) {
    ...
    if (selection != '') {
        //process selection
        var selection_as_array = selection.toString().split("\n"), new_selection = "";

        for (i = 0; i < selection_as_array.length; i++) {
            if (selection_as_array[i] != "") {
                new_selection += "<p>" + Typographus_Lite_UTF8.typo_text(selection_as_array[i].toString()) + "</p>";
            }
        }

        //        selection = Typographus_Lite_UTF8.typo_text(selection.toString());
        e.insertHtml(new_selection);
    }
